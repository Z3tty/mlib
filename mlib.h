...
extern uint_t vec3_compare(vec3_t v, vec3_t u)	{	if(v.x == u.x && v.y == u.y && v.z == u.z) return true; return false;	}
 ...
 * Since the arrays are null-terminated, we start at 1 tom get the correct length
 */
size_t length_str(string_t s) 	{ return strlen(s); 						}
size_t length_int(int i[]) 		{ uint_t c = 1; while(*i++) c++; return c; 	}
size_t length_flt(float f[])	{ uint_t c = 1; while(*f++) c++; return c; 	}
size_t length_sht(short s[])	{ uint_t c = 1; while(*s++) c++; return c;	}
size_t length_lng(long l[])		{ uint_t c = 1; while(*l++) c++; return c;	}
size_t length_dbl(double d[])	{ uint_t c = 1; while(*d++) c++; return c;	}
